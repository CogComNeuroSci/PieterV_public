...
pvoxels         = 990;
filenameneg     = strcat('ClusterStatisticFeedback_neg_',num2str(pvoxels),'.txt');
filenamepos     = strcat('ClusterStatisticFeedback_pos_',num2str(pvoxels),'.txt');
...
for rand = 1:nrandom
    ...
    for s = 1:num_subjects
        ...
        Feedback(idy) = Feedback(idy(randperm(num_trials)));
    end;
    
    Negative_indices= Feedback==0;
    Positive_indices=Feedback==1;

    % weigh power (standardized per subject) with SRPE (also standardized)
    % frequency * tijdspunt * electrode * (trial x subject)
