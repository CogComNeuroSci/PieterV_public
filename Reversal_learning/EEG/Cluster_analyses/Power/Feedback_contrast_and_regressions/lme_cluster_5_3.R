...
#Check distibution of PE
densityplot(~PE_estimate,data=Data_clean)
PE_distribution<-ggplot(data =Data_clean, aes(x=PE_estimate))+
                  geom_density(color = "black", fill = "darkblue", alpha=0.5)+
                  ggtitle("PE histogram")+
                  labs(x="Pe estimates")+
                  labs(y="Density")+
                  theme_classic()+
                  theme(axis.text = element_text(size=8, family="Times"), axis.title = element_text(size=8, family="Times"), plot.title = element_text(size = 8, face = "bold", hjust=0.5, family="Times"))+
                  scale_x_continuous(limits=c(-1,1))

ggsave(filename= paste(Figure_folder, "PE_distribution.tiff"), PE_distribution, width=5, height=5, units="cm", dpi=300)
...
eff = effect("PE_estimate:Reward", fit_theta_3, xlevels=list(PE_estimate=seq(-1,1, by=0.1)))
x.fit <- unlist(eff$x.all)
x <- data.frame(lower = eff$lower, upper = eff$upper, fit = eff$fit)
x$PE<-eff$x$PE_estimate
x$Reward<-eff$x$Reward
...
Theta_norew_plot_bis <- ggplot(x[x$Reward==0,], aes(x = PE, y = fit)) +
                                  theme_classic() +
                                  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5, fill = "blue", colour=NA)+
...
Theta_norew_plot_bis

Theta_rew_plot_bis <- ggplot(x[x$Reward==1,], aes(x = PE, y = fit)) +
  theme_classic() +
  geom_ribbon(aes(ymin = lower, ymax = upper), alpha = 0.5, fill = "blue", colour=NA)+
  geom_line(size = 1, colour="blue") +
  ggtitle("Unrewarded trials")+
