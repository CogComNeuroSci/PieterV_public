...
for(c in 1:length(cutoff-1)){
  for(sign in 1:2){
    
    name 			= paste("OverviewClustersFeedbackCutoff", cutoff[c], negpos[sign], ".txt", sep = "")
    ...
    for(clusi in 1:4){
      ...
      name = paste("Overview Clusters PLVplot Feedback Cutoff ", cutoff[c], " ", negpos[sign], " Cluster ", clusi, ".tiff", sep = "")
    ...
    name = paste("Overview Clusters Histogram Feedback Cutoff ", cutoff[c], " ", negpos[sign], ".tiff", sep = "")
