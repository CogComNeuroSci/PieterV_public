...
import estimation_Hybrid as estimation
import likelihood_Hybrid as likelihood
import sim_data_Hybrid as sim_data
...
for p in pplist:
    ...
    file = 'Behavioral_data_subject_{0}_Hybrid.csv'.format(int(p))
    ...
    print("Estimated parameters are {}, {} and {}".format(pars[loop][0], pars[loop][1], pars[loop][2]))
    
    sim_data.simulate_data(alpha = pars[loop][0], beta = pars[loop][1], decrease=pars[loop][2], file_name = file)
...
Decreases= [item[2] for item in pars]
...
print("Mean decrease")
print(np.mean(Decreases))
print("std decrease")
print(np.std(Decreases))
...
new_filename='Hybrid_output.csv'.format(int(p))
column_list = ["Subject", "Learning rate", "Temperature", "Decrease", "LogLik", "AIC", "BIC"]
    
output=pd.DataFrame({ 'Subject':pplist, 'Learning rate':Learning_rates, 'Temperature':Temperatures, "Decrease": Decreases, 'LogLik':Lik, 'AIC':AIC, 'BIC':BIC}, columns = column_list)
