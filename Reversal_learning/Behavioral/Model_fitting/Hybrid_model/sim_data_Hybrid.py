...
def simulate_data( alpha = 0.8, beta = 0.8, decrease=0.1,file_name = "sim_data.csv"):
    # initialize
    nstim=2
    nresp=2
    ...
    for loop in range(ntrials):
        ...
        if loop==0:
            ...
            learning_rate= decrease*abs(Reward-resp_act[resp])+(1-decrease)*learning_rate
            
        #Compute new values/weights
        value[stim,resp] = value[stim,resp] + alpha*(Reward-resp_act[resp]) *stim_act[stim]# Rescorla-Wagner update 
