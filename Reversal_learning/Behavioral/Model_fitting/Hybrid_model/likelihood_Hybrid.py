...
def logL(parameter, file_name):
    ...
    for trial_loop in range(ntrials):
        ...
        if trial_loop==0:
            ...
            learning_rate= parameter[2]*abs(data.iloc[trial_loop,4]-resp_act[resp])+(1-parameter[2])*learning_rate
            
        value[stim,resp] = value[stim,resp] + (learning_rate*(data.iloc[trial_loop,4]-resp_act[resp]) )*stim_act[stim]
