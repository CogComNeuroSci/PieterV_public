...
while np.mean(stim)!=0.5:
    for p in range(Nparts):
        s=np.repeat(np.arange(Ngratings),(math.ceil(mean_switch/Ngratings)))
        s=np.random.permutation(s)
        if p==0:
            stim[0:mean_switch]=s
        else:
            stim[mean_switch:Ntrials]=s
...
goodbye         = visual.TextStim(window,text=( "Dit is het einde van deze fase.\n\n"+
                                                ...
                                                "Geef een teken aan de afnemer dat je klaar bent \n\n").format(str(points)),wrapWidth=50, height=0.75, units="deg")
goodbye.draw()
window.flip()
event.waitKeys(keyList = "space")
