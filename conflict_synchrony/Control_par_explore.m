...
%Control parameters
Gamma=0.125:0.125:0.875;
Theta=0:0.125:1;
Beta=2.5:2.5:20;
Eta=0:0.125:1;

All_data=zeros(length(Gamma), length(Theta), length(Beta), length(Eta), Tr, 3);
Data_bis=zeros(length(Gamma)*length(Theta)*length(Beta)*length(Eta)*Tr,8);
...
if trial>1          %starting points are end points of previous trials
    ...
    for time = 1: ITI+RD    
        ...
        r2_acc=Control(1,trial)+Beta(B)*Energy(time,trial);
    ...
    Control(1,trial+1)=Gamma(G)*Control(1,trial)+(1-Gamma(G))*(Beta(B)*MAE(1,trial)+Eta(E));
    
    LFC(2,:,trial+1)=Control(1,trial+1)<Theta(Th);
    
end;
    
Data=Design;
All_data(G, Th, B, E, :,1)=rew';
All_data(G, Th, B, E,  :,2)=RT';
All_data(G, Th, B, E,  :,3)=Design(:,3);

Data_bis((sim-1)*Tr+1:sim*Tr,1)=sim;
Data_bis((sim-1)*Tr+1:sim*Tr,2)=Gamma(G);
Data_bis((sim-1)*Tr+1:sim*Tr,3)=Theta(Th);
Data_bis((sim-1)*Tr+1:sim*Tr,4)=Beta(B);
Data_bis((sim-1)*Tr+1:sim*Tr,5)=Eta(E);
Data_bis((sim-1)*Tr+1:sim*Tr,6)=rew';
Data_bis((sim-1)*Tr+1:sim*Tr,7)=RT';
Data_bis((sim-1)*Tr+1:sim*Tr,8)=Design(:,3);
            end;
        end;
    end;
end;

save('Control_pars_tres', 'All_data')

fid = fopen('Data_control_pars_tres.txt','wt');
