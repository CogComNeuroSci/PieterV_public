...
Directory = "/Volumes/backupdisc/Modular_learning/Data_Revision/MNIST/gatelayer/"
learning_rates= np.arange(0,0.11,0.02)
Rep= 25
Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"]
nContexts= 6

def load_data(Directory = "/Volumes/backupdisc/Modular_learning/Data_Revision/MNIST/gatelayer/", learning_rates= np.arange(0,0.11,0.02), Rep= 25, Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"], resources = 400, nContexts= 6, nPatterns = 10, g = 0):

    Accuracy_labeled = np.zeros((len(Models), len(learning_rates), Rep, nContexts, nPatterns))
    Activation_labeled =  np.zeros((len(Models), len(learning_rates), Rep, resources+2, nContexts, nPatterns))
    ...
    for m in Models:
        i+=1
        data = np.load(Directory + m + "/Gat{:d}_Generalization_data.npy".format(g), allow_pickle = True)
...
data_dict = {}
for g in range(4):
    data_dict[str(g)] = load_data(Directory = Directory, g = g)
...
mean_accuracy_0 = np.mean(np.mean(accuracy_0,4),2)*100
std_accuracy_0 = np.std(np.mean(accuracy_0,4), axis = 2)*100
...
MNIST_accuracy_gen_dir = {
    "0_Hidden_mean": mean_accuracy_0,
    "0_Hidden_ci": ci_accuracy_0,
...
Overlap_all_0 = np.zeros((len(Models), len(learning_rates), Rep, nContexts, nContexts, nLabels))
...
Overlap_0layer1 =  np.zeros((len(Models), len(learning_rates), Rep, nContexts, nContexts, nLabels))
Overlap_0layer2 =  np.zeros((len(Models), len(learning_rates), Rep, nContexts, nContexts, nLabels))

Overlap_1layer1 =  np.zeros((len(Models), len(learning_rates), Rep, nContexts, nContexts, nLabels))
Overlap_1layer2 =  np.zeros((len(Models), len(learning_rates), Rep, nContexts, nContexts, nLabels))
...
Overlap_all0_average = np.mean(Overlap_all_0,5)
...
Overlap_0layer1_average = np.mean(Overlap_0layer1,5)
Overlap_0layer2_average = np.mean(Overlap_0layer2,5)

Overlap_1layer1_average = np.mean(Overlap_1layer1,5)
Overlap_1layer2_average = np.mean(Overlap_1layer2,5)
...
Correlation_all_0 = np.zeros((len(Models), len(learning_rates), Rep))
...
Correlation_0layer1 = np.zeros((len(Models), len(learning_rates), Rep))
Correlation_0layer2 = np.zeros((len(Models), len(learning_rates), Rep))

Correlation_1layer1 = np.zeros((len(Models), len(learning_rates), Rep))
Correlation_1layer2 = np.zeros((len(Models), len(learning_rates), Rep))
...
for m in range(len(Models)):
    for l in range(len(learning_rates)):
        for r in range(Rep):
            actual_all_0 = np.reshape(Overlap_all0_average[m,l,r,:,:], (-1))
            ...
            actual_0layer1 = np.reshape(Overlap_0layer1_average[m,l,r,:,:], (-1))
            actual_0layer2 = np.reshape(Overlap_0layer2_average[m,l,r,:,:], (-1))

            actual_1layer1 = np.reshape(Overlap_1layer1_average[m,l,r,:,:], (-1))
            actual_1layer2 = np.reshape(Overlap_1layer2_average[m,l,r,:,:], (-1))

            actual_2layer1 = np.reshape(Overlap_2layer1_average[m,l,r,:,:], (-1))
            actual_2layer2 = np.reshape(Overlap_2layer2_average[m,l,r,:,:], (-1))
            ...
            Correlation_all_0[m,l,r] = stats.spearmanr(actual_all_0, target_0)[0]
            ...
            Correlation_0layer1[m,l,r] = stats.spearmanr(actual_0layer1, target_0)[0]
            Correlation_0layer2[m,l,r] = stats.spearmanr(actual_0layer2, target_0)[0]

            Correlation_1layer1[m,l,r] = stats.spearmanr(actual_1layer1, target_1)[0]
            Correlation_1layer2[m,l,r] = stats.spearmanr(actual_1layer2, target_1)[0]
...
MNIST_RDM_gen_dir = {
    "0_Hidden_mean": Correlation_all0_mean,
    "0_Hidden_ci": Correlation_all0_ci,
    ...
    "0_1_Hidden_mean": Correlation_0layer1_mean,
    "0_1_Hidden_ci": Correlation_0layer1_ci,
    "0_2_Hidden_mean": Correlation_0layer2_mean,
    "0_2_Hidden_ci": Correlation_0layer2_ci,
    "1_1_Hidden_mean": Correlation_1layer1_mean,
    "1_1_Hidden_ci": Correlation_1layer1_ci,
    "1_2_Hidden_mean": Correlation_1layer2_mean,
    "1_2_Hidden_ci": Correlation_1layer2_ci,
...
pca_0 = np.zeros((len(Models),len(learning_rates),Rep, 2, nContexts,nLabels))
...
pca_0_1 = np.zeros((len(Models),len(learning_rates),Rep, 2, nContexts,nLabels))
pca_0_2 = np.zeros((len(Models),len(learning_rates),Rep, 2, nContexts,nLabels))

pca_1_1 = np.zeros((len(Models),len(learning_rates),Rep, 2, nContexts,nLabels))
pca_1_2 = np.zeros((len(Models),len(learning_rates),Rep, 2, nContexts,nLabels))
...
pca_0_explained = np.zeros((len(Models),len(learning_rates),Rep, 2))
...
for m in range(len(Models)):
    for lr in range(len(learning_rates)):
        for r in range(Rep):
            ...
            pca_0[m,lr,r,:,:,:] = np.reshape(np.transpose(p.fit_transform(data_0)),(2,nContexts,nLabels))
            PCA(n_components = 2)
            pca_0_explained[m,lr,r,:]=p.explained_variance_ratio_
            ...
            pca_0_1[m,lr,r,:,:,:] = np.reshape(np.transpose(p.fit_transform(data_0_1)),(2,nContexts,nLabels))
            pca_0_2[m,lr,r,:,:,:] = np.reshape(np.transpose(p.fit_transform(data_0_2)),(2,nContexts,nLabels))

            pca_1_1[m,lr,r,:,:,:] = np.reshape(np.transpose(p.fit_transform(data_1_1)),(2,nContexts,nLabels))
            pca_1_2[m,lr,r,:,:,:] = np.reshape(np.transpose(p.fit_transform(data_1_2)),(2,nContexts,nLabels))
...
print(np.mean(np.reshape(np.sum(pca_0_explained,3),-1)))
print(np.std(np.reshape(np.sum(pca_0_explained,3),-1)))
