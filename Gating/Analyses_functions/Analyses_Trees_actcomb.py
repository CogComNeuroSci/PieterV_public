...
def load_data(Directory = "/Volumes/backupdisc/Modular_learning/Data_Revision/comb/", learning_rates= np.arange(0,1.1,0.2), Rep= 25, Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"], resources = 12, nContexts= 4, nRepeats= 3, nPatterns = 4, act = ["sig", "relu"]):
    ...
    for m in Models:
        ...
        for lr in learning_rates:
            ...
            for r in range(Rep):
                if "add" in m:
                    data = np.load(Directory + "Trees/"+ m +"/" + act[0] +"_lr_{:.1f}_Rep_{:d}.npy".format(lr, r), allow_pickle = True)
                else:
                    data = np.load(Directory + "Trees/"+ m +"/" + act[0] + "_" + act[1] + "_lr_{:.1f}_Rep_{:d}.npy".format(lr, r), allow_pickle = True)
                print([i, i2, r])
                ...
                for n in range(nPatterns):
                    for re in range(nRepeats):
                        id1 = P[n][re]
                        for c in range(nContexts):
                            id2 = data[()]["Contextorder"]==c
                            Accuracy_labeled[i,i2,r,re,c,n]= np.mean(data[()]["Accuracy"][re,id2,id1])
                            Activation_labeled[i,i2,r,:,re,c,n]=np.mean(data[()]["Activation"][:,re,id2,id1],1)
...
if not loaded:
    for i in ["sig", "relu"]:
        for j in ["sig", "relu"]:
            ...
            np.save(Directory+"all_data_Trees_"+i+j+".npy", new_dict)

#Now extract the loaded data
data_dict = {}
for i in ["sig", "relu"]:
    for j in ["sig", "relu"]:
        data_dict[i+j] = np.load(Directory+"all_data_Trees_"+i+j+".npy", allow_pickle = True)
...
figdir = "/Volumes/backupdisc/Modular_learning/Plots_Trees/Revision/comb/"
...
np.save(Directory + "Accuracy_train_Trees.npy", Trees_accuracy_train_dir)
...
plt.title("Model performance on Trees task")
...
for i in range(3):
    for m in range(len(Models)):
        axs[i,0].errorbar(np.arange(1,13), reshaped_mean_acc_sigsig[m,i+1,:], reshaped_ci_acc_sigsig[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,1].errorbar(np.arange(1,13), reshaped_mean_acc_sigrelu[m,i+1,:], reshaped_ci_acc_sigrelu[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,2].errorbar(np.arange(1,13), reshaped_mean_acc_relurelu[m,i+1,:], reshaped_ci_acc_relurelu[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,3].errorbar(np.arange(1,13), reshaped_mean_acc_relusig[m,i+1,:], reshaped_ci_acc_relusig[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
    for i2 in range(4):
        axs[i,i2].spines['top'].set_visible(False)
        axs[i,i2].spines['right'].set_visible(False)
        axs[2,i2].set_xticks(np.arange(1,13))
        axs[2,i2].set_xticklabels(np.tile(["L","Br","and","xor"],3), fontsize = 9)
...
np.save(Directory + "Criterion_train_Trees.npy", Trees_criterion_train_dir)
...
np.save(Directory + "RDM_train_Trees.npy", Trees_RDM_train_dir)
...
np.save(Directory + "PCA_train_Trees.npy", Trees_pca_train_dir)
