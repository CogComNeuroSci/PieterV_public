...
def load_data(Directory = "/Volumes/backupdisc/Modular_learning/Data_Revision/comb/", learning_rates= np.arange(0,1.1,0.2), Rep= 25, Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"], resources = 12, nContexts= 4, nRepeats= 3, nPatterns = 4, act = ["sig", "relu"]):
    ...
    for m in Models:
        i+=1
        if "add" in m:
            data = np.load(Directory + "Trees/"+ m +"/Generalization_data_" + act[0] +".npy", allow_pickle = True)
        else:
            data = np.load(Directory + "Trees/"+ m +"/Generalization_data_" + act[0] + "_" + act[1] + ".npy", allow_pickle = True)
...
for i in range(2):
    ...
    for j in range(4):
        ...
        axs[1,j].set_xticklabels(["L","Br","and","xor"], fontsize = 9)
...
for m in range(len(Models)):
    for lr in range(len(learning_rates)):
        for r in range(Rep):
            ...
            PCA(n_components = 2)
            pca_sigrelu_explained[m,lr,r,:]=p.explained_variance_ratio_
            pca_relurelu[m,lr,r,:,:,:] = np.reshape(np.transpose(p.fit_transform(data_relurelu)),(2,nContexts,nPatterns))
            PCA(n_components = 2)
            pca_relurelu_explained[m,lr,r,:]=p.explained_variance_ratio_
            pca_relusig[m,lr,r,:,:,:] = np.reshape(np.transpose(p.fit_transform(data_relusig)),(2,nContexts,nPatterns))
            PCA(n_components = 2)
            pca_relusig_explained[m,lr,r,:]=p.explained_variance_ratio_

print(np.mean(np.reshape(np.sum(pca_sigsig_explained,3),-1)))
print(np.std(np.reshape(np.sum(pca_sigsig_explained,3),-1)))
print(np.mean(np.reshape(np.sum(pca_sigrelu_explained,3),-1)))
print(np.std(np.reshape(np.sum(pca_sigrelu_explained,3),-1)))
print(np.mean(np.reshape(np.sum(pca_relusig_explained,3),-1)))
print(np.std(np.reshape(np.sum(pca_relusig_explained,3),-1)))
print(np.mean(np.reshape(np.sum(pca_relurelu_explained,3),-1)))
print(np.std(np.reshape(np.sum(pca_relurelu_explained,3),-1)))
