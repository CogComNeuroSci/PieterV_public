...
Directory = "/Volumes/backupdisc/Modular_learning/Data_Revision/Trees/"
...
def load_data(Directory = "/Volumes/backupdisc/Modular_learning/Data_Revision/Trees/", learning_rates= np.arange(0,1.1,0.2), Rep= 25, Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"], resources = 24, nContexts= 4, nRepeats= 3, nPatterns = 4):
...
for i in range(3):
    for m in range(len(Models)):
        axs[i,0].errorbar(np.arange(1,13), reshaped_mean_acc_12[m,i+1,:], reshaped_ci_acc_12[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,1].errorbar(np.arange(1,13), reshaped_mean_acc_24[m,i+1,:], reshaped_ci_acc_24[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
    for i2 in range(2):
