...
def load_data(Directory = "/Volumes/backupdisc/Modular_learning/Data_Revision/", learning_rates= np.arange(0,1.1,0.2), Rep= 25, Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"], resources = 12, nContexts= 4, nRepeats= 3, nPatterns = 4, method = "conc"):
    ...
    for m in Models:
        ...
        for lr in learning_rates:
            i2+=1
            for r in range(Rep):
                data = np.load(Directory + method + "/Trees/"+ m +"/lr_{:.1f}_Rep_{:d}.npy".format(lr, r), allow_pickle = True)
                print([i, i2, r])
...
figdir = "/Volumes/backupdisc/Modular_learning/Plots_Trees/Revision/consep/"
...
for i in range(3):
    for m in range(len(Models)):
        axs[i,0].errorbar(np.arange(1,13), reshaped_mean_acc_conc[m,i+1,:], reshaped_ci_acc_conc[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,1].errorbar(np.arange(1,13), reshaped_mean_acc_sep[m,i+1,:], reshaped_ci_acc_sep[m,i+1,:], lw = 2, color=color_values[m], label=model_labels[m])
    for i2 in range(2):
