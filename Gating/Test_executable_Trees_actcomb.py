...
from Preparation_functions import Prep_Trees_test_actcomb as P
from Model_functions import model_test_fun_actcomb as T


#Define setings of previous simulations
resources = 12
Dir = "/Volumes/backupdisc/Modular_learning/Data_Revision/comb/Trees/" #"/data/gent/430/vsc43099/GatingModel_data/"
    ...
    for m in range(2):
        ...
        for lr in learning_rates:
            ...
            for r in range(Rep):
                ...
                if core < 2:
                    #Prepare data
                    Dat = P.Generalization_preparation(nContexts=4, Directory = Dir, Model=Models[m+2], Part_trials = 100, lr=lr, r=r, resources=resources, act = act)
                    #Simulate network
                    Result = T.Generalization_additive(Dat["Inputs"], Dat["Contexts"], Dat["Input_weights"], Dat["Output_weights"], Dat["CorResp"], False, act)

                else:
                    #Prepare data
                    Dat = P.Generalization_preparation(nContexts=4, Directory = Dir, Model=Models[m], Part_trials = 100, lr=lr, r=r, resources=resources, act = act)
                    #Simulate network
                    Result = T.Generalization_multiplicative(Dat["Inputs"], Dat["Contexts"], Dat["Context_weights"], Dat["Input_weights"], Dat["Output_weights"], Dat["CorResp"], False, act)
