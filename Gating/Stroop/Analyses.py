...
def load_data(Directory = "/Volumes/backupdisc/Modular_learning/Data_Stroop/", learning_rates= np.arange(0,1.1,0.1), Rep= 50, Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"], resources = 24, nContexts= 5, nRepeats= 3, nPatterns = 18):
    ...
    return Contextorder, Overlap, Accuracy_labeled, Activation_labeled

if not loaded:
    for i in [12, 24]:
        order, target_overlap, accuracy, activation = load_data(resources = i)
        new_dict = {
            "order": order,
...
for i in range(3):
    for m in range(len(Models)):
        axs[i,0].errorbar(np.arange(1,16), reshaped_mean_acc_12[m,(i+1)*3,:], reshaped_ci_acc_12[m,(i+1)*3,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,1].errorbar(np.arange(1,16), reshaped_mean_acc_24[m,(i+1)*3,:], reshaped_ci_acc_24[m,(i+1)*3,:], lw = 2, color=color_values[m], label=model_labels[m])
    for i2 in range(2):
    ...
    for l in range(len(learning_rates)):
        for r in range(Rep):
            for nr in range(nRepeats):
                ...
                target_24 = np.reshape(target_overlap_12[m,l,r,:,:], (-1))
                Correlation_12[m,l,r,nr] = np.corrcoef(actual_12, target_12)[0,1]
                Correlation_24[m,l,r,nr] = np.corrcoef(actual_24, target_24)[0,1]
