...
def Generalization_preparation(nContexts = 5, nRepeats = 3, Directory = "", Model="Adaptive_mult", ntrep = 25, lr=0, r=0, resources=12, act = ["sig", "sig"]):

    #Set random seed for replicability
    np.random.seed(r+100)

    #Define input patterns and corresponding objectives
    nInput = 8
    nOutput = 3
    ...
    if "add" in Model:
        Data = np.load(Directory + Model +"/" + act[0]+"_lr_{:.1f}_Rep_{:d}.npy".format(lr, r), allow_pickle=True)
    else:
        Data = np.load(Directory + Model +"/" + act[0]+"_"+act[1]+"_lr_{:.1f}_Rep_{:d}.npy".format(lr, r), allow_pickle=True)
    ...
    if "add" in Model:
        Output = {
            ...
            "Stim_labels": np.tile(np.arange(nPatterns), int(Part_trials/nPatterns))
            }
    else:
        Output = {
        "Inputs": Inputs,
