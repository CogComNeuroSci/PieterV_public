...
from Preparation_functions import Prep_MNIST_test as P
from Model_functions import model_test_fun_basic as T

Dir = "/Volumes/backupdisc/Modular_learning/Data_Revision/MNIST/onelayer/"#"/data/gent/430/vsc43099/GatingModel_data/"
...
def Gen_fun(core=0):
    ...
    Result_dictionary={
        ...
        "Activation": np.zeros((np.size(learning_rates),Rep, 401, 6, 500)),
    ...
    for lr in learning_rates:
        ...
        for r in range(Rep):
            ...
            if core<2:
                Result = T.Generalization_multiplicative(Dat["Inputs"], Dat["Contexts"], Dat["Context_weights"], Dat["Input_weights"], Dat["Output_weights"], Dat["CorResp"], multout= False)
                print(r)
                print(lr)
            else:
                Result = T.Generalization_additive(Dat["Inputs"], Dat["Contexts"], Dat["Input_weights"], Dat["Output_weights"], Dat["CorResp"], multout = False)
