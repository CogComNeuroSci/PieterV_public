...
from Preparation_functions import Prep_MNIST_test_gatelayer as P
from Model_functions import model_test_fun_gatlayer_2 as T

Dir = "/Volumes/backupdisc/Modular_learning/Data_Revision/MNIST/gatelayer/"#"/data/gent/430/vsc43099/GatingModel_data/"
    ...
    for g in range(4):
        ...
        for lr in learning_rates:
            ...
            for r in range(Rep):
                ...
                Dat = P.Generalization_preparation(nContexts = 6, Directory = Dir, Model=Models[core], Trial_percentage = 0.05, lr=lr, r=r, xdat = x_test, ydat = y_test, g=g)

                if core<2:
                    Result = T.Generalization_multiplicative(Dat["Inputs"], Dat["Contexts"], Dat["Context_weights"], Dat["Input_weights"], Dat["Hidden_weights"], Dat["Output_weights"], Dat["CorResp"], g)
                    print("mult")
                else:
                    Result = T.Generalization_additive(Dat["Inputs"], Dat["Contexts"], Dat["Input_weights"], Dat["Hidden_weights"], Dat["Output_weights"], Dat["CorResp"],g)
                    print("add")
        ...
        np.save(Dir + Models[core] + "/Gat{:d}_Generalization_data".format(g), Result_dictionary)
