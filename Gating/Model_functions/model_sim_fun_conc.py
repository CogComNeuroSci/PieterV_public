        ...
        for c in range(nContexts):
            ...
            for t in range(Part_trials):
                ...
                if multout:
                    ...
                    In = Inputs[r,t,:]
                C = Contexts[c,:]
                Hidden[:nHidden-1]=mult_activation(In, W_inp_hid, C, W_cont_hid, 0)
                Out = single_activation(Hidden, W_hid_out, 0)
                ...
                update_inp_hid = learning_rate * In.reshape((nInput,1)) @ ((((Obj-Out) * Out * (1-Out)) @ np.transpose(W_hid_out[:-1,:])) * (C @ W_cont_hid) * Hidden[:nHidden-1] * (1-Hidden[:nHidden-1])).reshape((1,nHidden-1))
                if gate_learning:
                    update_cont_hid = learning_rate * C.reshape((nContexts,1)) @ ((((Obj-Out) * Out * (1-Out)) @ np.transpose(W_hid_out[:-1,:])) * (In @ W_inp_hid)* Hidden[:nHidden-1]* (1-Hidden[:nHidden-1])).reshape((1,nHidden-1))
                else:
        ...
        for c in range(nContexts):
            ...
            for t in range(Part_trials):
                ...
                if multout:
                    ...
                    In = Inputs[r,t,:]
                C = Contexts[c,:]
                Hidden[:nHidden-1]=add_activation(In, W_inp_hid, C, W_cont_hid, 0)
                Out = single_activation(Hidden, W_hid_out, 0)
                ...
                #Compute weight updates
                update_hid_out= learning_rate * Hidden.reshape((nHidden,1)) @ ((Obj-Out) * Out * (1-Out)).reshape((1,nOutput))
                update_inp_hid = learning_rate * In.reshape((nInput,1)) @ ((((Obj-Out) * Out * (1-Out)) @ np.transpose(W_hid_out[:-1,:])) * Hidden[:-1] * (1-Hidden[:-1])).reshape((1,nHidden-1))
                if gate_learning:
                    update_cont_hid = learning_rate * C.reshape((nContexts,1)) @ ((((Obj-Out) * Out * (1-Out)) @ np.transpose(W_hid_out[:-1,:])) * Hidden[:-1] * (1-Hidden[:-1])).reshape((1,nHidden-1))
                else:
                    update_cont_hid = np.zeros((nContexts,nHidden-1))
