...
def Generalization_multiplicative(Inputs, Contexts, Context_weights, Input_weights, Hidden_weights, Output_weights, Objectives):
    ...
    nHidden1 = np.size(Hidden_weights,0)
    nHidden2 = np.size(Output_weights,0)
    nOutput =1
    ...
    Activation = np.zeros((nHidden1+nHidden2, nContexts, np.size(Inputs,0)))
    ...
    for c in range(nContexts):
        for t in range(np.size(Inputs,0)):
            ...
            Hidden2[:nHidden2-1] = sigmoid_activation(Hidden1, Hidden_weights, 0)
