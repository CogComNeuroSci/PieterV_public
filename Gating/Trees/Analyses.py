    ...
    for m in Models:
        ...
        for lr in learning_rates:
            ...
            for r in range(Rep):
                ...
                print([i, i2, r])
                # Define 4 quadrants of input space
                Inputs = data[()]["Presented"]
                P = []
                P.append(((Inputs[:,0]>0.5)*1 + (Inputs[:,1]>0.5)*1)*1)
                P.append(((Inputs[:,0]>0.5)*1 + (Inputs[:,1]<0.5)*1)*1)
                P.append(((Inputs[:,0]<0.5)*1 + (Inputs[:,1]>0.5)*1)*1)
                P.append(((Inputs[:,0]<0.5)*1 + (Inputs[:,1]<0.5)*1)*1)
...
for i in range(3):
    for m in range(len(Models)):
        axs[i,0].errorbar(np.arange(1,13), reshaped_mean_acc_12[m,(i+1)*3,:], reshaped_ci_acc_12[m,(i+1)*3,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,1].errorbar(np.arange(1,13), reshaped_mean_acc_24[m,(i+1)*3,:], reshaped_ci_acc_24[m,(i+1)*3,:], lw = 2, color=color_values[m], label=model_labels[m])
    for i2 in range(2):
...
contexts=["Leafy","Branchy","and","xor"]
color_bis = ["c", "m", "y", "b","k"]
