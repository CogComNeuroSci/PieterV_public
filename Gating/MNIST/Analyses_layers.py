...
def load_data(Directory = "/Volumes/backupdisc/Modular_learning/Data_MNIST/", learning_rates= np.arange(0,0.11,0.01), Rep= 50, Models= ["Adaptive_mult", "Non_adaptive_mult", "Adaptive_add", "Non_adaptive_add"], nContexts= 6, nRepeats= 3, layers=2):
    Accuracy_numbered_unshuffled = np.zeros((len(Models), len(learning_rates), Rep, nRepeats, nContexts, 10))
    Activation_numbered_unshuffled =  np.zeros((len(Models), len(learning_rates), Rep, 400+layers, nRepeats, nContexts, 10))
    ...
    for m in Models:
        ...
        for lr in learning_rates:
            ...
            for r in range(Rep):
                ...
                Labels[i,i2,r,:,:] = data[()]["Presented_numbers"]
                for n in np.unique(data[()]["Presented_numbers"]):
                    for r2 in range(nRepeats):
                        id1 = data[()]["Presented_numbers"][r2,:]==n
                        for c in range(nContexts):
                            id2 = data[()]["Contextorder"]==c
                            Accuracy_numbered_unshuffled[i,i2,r,r2,:,n]=np.mean(data[()]["Accuracy"][r2,id2,id1])
                            Activation_numbered_unshuffled[i,i2,r,:,r2,c,n]=np.mean(data[()]["Activation"][:,r2,id2,id1],1)


    return Contextorder, Overlap, Labels, Accuracy_numbered_unshuffled, Activation_numbered_unshuffled
...
for i in range(3):
    for m in range(len(Models)):
        axs[i,0].errorbar(np.arange(1,19), reshaped_mean_acc_1[m,(i+1)*3,:], reshaped_ci_acc_1[m,(i+1)*3,:], lw = 2, color=color_values[m], label=model_labels[m])
        axs[i,1].errorbar(np.arange(1,19), reshaped_mean_acc_2[m,(i+1)*3,:], reshaped_ci_acc_2[m,(i+1)*3,:], lw = 2, color=color_values[m], label=model_labels[m])
...
fig.text(0.01, 0.77, '\u03B1 = 0.03', va='center', rotation='vertical', fontsize=9, fontweight = "bold")
fig.text(0.01, 0.5, '\u03B1 = 0.06', va='center', rotation='vertical', fontsize=9, fontweight = "bold")
fig.text(0.01, 0.23, '\u03B1 = 0.09', va='center', rotation='vertical', fontsize=9, fontweight = "bold")
...
Overlap_layer1 =  np.zeros((len(Models), len(learning_rates), Rep, nRepeats, nContexts, nContexts, nLabels))
Overlap_layer2 =  np.zeros((len(Models), len(learning_rates), Rep, nRepeats, nContexts, nContexts, nLabels))
...
Overlap_layer1_average = np.mean(Overlap_layer1,6)
Overlap_layer2_average = np.mean(Overlap_layer2,6)
...
Correlation_layer1 = np.zeros((len(Models), len(learning_rates), Rep, nRepeats))
Correlation_layer2 = np.zeros((len(Models), len(learning_rates), Rep, nRepeats))
    ...
    for l in range(len(learning_rates)):
        for r in range(Rep):
            for nr in range(nRepeats):
                ...
                actual_layer1 = np.reshape(Overlap_layer1_average[m,l,r,nr,:], (-1))
                actual_layer2 = np.reshape(Overlap_layer2_average[m,l,r,nr,:], (-1))
                ...
                Correlation_layer1[m,l,r,nr] = np.corrcoef(actual_layer1, target_2)[0,1]
                Correlation_layer2[m,l,r,nr] = np.corrcoef(actual_layer2, target_2)[0,1]
...
MNIST_RDM_train_dir = {
    ...
    "2_1_Hidden_mean": np.mean(Correlation_layer1_mean,1),
    "2_1_Hidden_ci": np.mean(Correlation_layer1_ci,1),
    "2_2_Hidden_mean": np.mean(Correlation_layer2_mean,1),
    "2_2_Hidden_ci": np.mean(Correlation_layer2_ci,1)
}
...
for m in range(len(Models)):
    ...
    axs[2,0].errorbar(learning_rates, np.mean(Correlation_layer1_mean[m,:,:],1),  np.mean(Correlation_layer1_ci[m,:,:],1), lw = 2, color=color_values[m], label=model_labels[m])
    axs[3,0].errorbar(learning_rates, np.mean(Correlation_layer2_mean[m,:,:],1),  np.mean(Correlation_layer2_ci[m,:,:],1), lw = 2, color=color_values[m], label=model_labels[m])
    ...
    axs[2,1].errorbar(np.arange(1,nRepeats+1), np.mean(Correlation_layer1_mean[m,:,:],0),  np.mean(Correlation_layer1_ci[m,:,:],0), lw = 2, color=color_values[m], label=model_labels[m])
    axs[3,1].errorbar(np.arange(1,nRepeats+1), np.mean(Correlation_layer2_mean[m,:,:],0),  np.mean(Correlation_layer2_ci[m,:,:],0), lw = 2, color=color_values[m], label=model_labels[m])
