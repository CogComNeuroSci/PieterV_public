...
#Empirical analyses (ERN, PAC, time-frequency) only in matlab codes
...
for b in range(betas):
    for r in range(Rep):
        for i in range(2):
            myworkspaceconn = pickle.load(open("Beta"+str(b/10)+"_Rep"+str(r)+"_backprop_nosync.p", "rb"))
            
        Accuracy_conn[:,b,r]=np.array(list(myworkspaceconn[1].values()))*100
        ERR_conn[:,b,r]=np.array(list(myworkspaceconn[0].values()))
        
        for i in range(2):
            myworkspacesync = pickle.load(open("Beta"+str(b/10)+"_Rep"+str(r)+"_backprop_sync.p", "rb"))
